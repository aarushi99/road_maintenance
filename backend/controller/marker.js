...
exports.postMarker = (req, res, next) => {
  ...
  const creator = req.body.creator ? req.body.creator : "Road Maintenance App";
...
exports.deleteMarker = (req, res, next) => {
  MarkerModel.deleteOne({ _id: req.params.mId })
    .then((result) => {
      console.log("Marker : ", req.params.mId, " successfully deleted");
      HistoryModel.deleteMany({ markerId: req.params.mId })
        .then((result) => {
          console.log("Marker History successfully deleted");
          res.status(202).json({
